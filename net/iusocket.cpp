...
{
    LOG_INFO("CIUSocket::SetRecvMsgThread(CRecvMsgThread* pThread)");
    m_pRecvMsgThread = pThread;
}

bool CIUSocket::Init()
{
    LOG_INFO("CIUSocket::Init()  ");
    m_bStop = false;
...
{
    LOG_INFO("CIUSocket::SetServer(PCTSTR lpszServer)");
...
{
    LOG_INFO(" CIUSocket::SetPort(short nPort)");
    m_nPort = nPort;
    CloseFileServerConnection();
}
...
{
    LOG_INFO(" CIUSocket::EnableReconnect(bool bEnable)");
    m_bEnableReconnect = bEnable;
}

bool CIUSocket::Connect(int timeout /*= 3*/)
{
    LOG_INFO(" CIUSocket::Connect(int timeout /*= 3*/)");
    //Close();
...
{
    LOG_INFO(" CIUSocket::ReConnect(int timeout /*= 3*/)");
...
{
    LOG_INFO("CIUSocket::ConnectToFileServer");
...
{
    LOG_INFO("CIUSocket::CheckReceivedData()");
    fd_set readset;
    FD_ZERO(&readset);
    ...
    LOG_INFO("CIUSocket::CheckReceivedData()    --->    start1");
    long nRet = ::select(m_hSocket + 1, &readset, NULL, &exceptionset, &timeout);
    LOG_INFO("CIUSocket::CheckReceivedData()  nRet  --->  %d", nRet);
    if (nRet >= 1)
    {
        LOG_INFO("nRet >= 1     ... ...");
...
bool CIUSocket::Send()
{
    ...
    LOG_INFO("m_strSendBuf --> %s, m_strSendBuf.length --> %d ", m_strSendBuf.c_str(), m_strSendBuf.length());
...
{
    LOG_INFO("CIUSocket::SendThreadProc()");
    LOG_INFO("Send data thread start...");
    ...
    LOG_INFO("Send data thread finish...");
}

void CIUSocket::Send(const std::string& strBuffer)
{
    LOG_INFO("CIUSocket::Send(const std::string& strBuffer)  ...");
    //size_t nDestLength;
    ...
    LOG_INFO("strDestBuf -->  %s, strDestBuf.length --> %d", strDestBuf.c_str(), strDestBuf.length());
...
void CIUSocket::RecvThreadProc()
{
    ...
    while (!m_bStop)
    {
        ...
        else if (nRet == 0)
        {
            ...
            //LOG_INFO("bAlive    --> %d", bAlive);
            if (!bAlive && ::GetLastError() == 0)
            {
                //LOG_ERROR("else if (nRet == 0) -- 1 ....    ....    ");
                //网络已经断开
                m_pRecvMsgThread->NotifyNetError();
...
//            LOG_INFO("m_nLastDataTime  -->     %d", nLastDataTime);
//            LOG_INFO("m_nHeartbeatInterval  -->     %d", m_nHeartbeatInterval);

            //LOG_INFO("time(NULL) - nLastDataTime    -->     %d",time(NULL) - nLastDataTime);

            if (m_nHeartbeatInterval > 0)
            {
//                LOG_ERROR("else if (nRet == 0) -- 3 ....    ....    ");
                 LOG_INFO("time(NULL) - nLastDataTime    -->     %d",time(NULL) - nLastDataTime);
...
{
    LOG_INFO("CIUSocket::DecodePackages()");
...
bool CIUSocket::Login(const char* pszUser, const char* pszPassword, int nClientType, int nOnlineStatus, int nTimeout, std::string& strReturnData)
{
    ...
    LOG_INFO("strSendBuf.length --> %d, strSendBuf --> %s", strSendBuf.length(), strSendBuf.c_str());
...
{
    LOG_INFO("CIUSocket::SendData ...");
    LOG_INFO("strSendData.length --> , strSendData --> %s", pBuffer);
...
{
    LOG_INFO("CIUSocket::RecvData  ....");
    LOG_INFO("pszBuff --> %s", pszBuff);
    int64_t nStartTime = time(NULL);
...
{
    LOG_INFO("CIUSocket::SendHeartbeatPackage()");
    std::string outbuf;
    net::BinaryStreamWriter writeStream(&outbuf);
