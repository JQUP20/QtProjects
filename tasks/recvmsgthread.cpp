...
#include "UserData/flamingoclient.h"
...
{
    LOG_INFO(" FormatContent(std::vector<CContent*>& arrContent)");
...
{
    LOG_INFO("CreateMsgLogFile(CMessageLogger& msgLogger, const tstring& strFileName)");
...
m_bNetError(false)
{
    LOG_INFO("CRecvMsgThread::CRecvMsgThread() :");
}
...
{
    LOG_INFO("CRecvMsgThread::AddMsgData(const std::string& pMsgData) ...");
    if (pMsgData.empty())
        return;
...
{
    LOG_INFO("CRecvMsgThread::NotifyNetError()");
    //已经有错误了，就不要发送重复消息了
    if (!m_bNetError)
    {
        m_bNetError = true;
...
{
    LOG_INFO("CRecvMsgThread::HandleMessage(const std::string& strMsg)");
...
{
    LOG_INFO("CRecvMsgThread::HandleRegisterMessage(const std::string& strMsg)");
...
{
    LOG_INFO("CRecvMsgThread::HandleLoginMessage(const std::string& strMsg)");
    //{"code": 0, "msg": "ok", "userid": 8}
    Json::Reader JsonReader;
    Json::Value JsonRoot;
    if (!JsonReader.parse(strMsg, JsonRoot))
...
{
    LOG_INFO("CRecvMsgThread::HandleFriendListInfo(const std::string& strMsg)  ...... ");
    /*
    {
    "code": 0,
    "msg": "ok",
...
{
    LOG_INFO("CRecvMsgThread::HandleGroupBasicInfo(const std::string& strMsg)  ...... ");
    Json::Reader JsonReader;
    Json::Value JsonRoot;
    if (!JsonReader.parse(strMsg, JsonRoot))
    {
...
RMT_BUDDY_DATA* CRecvMsgThread::GetBuddyData(UINT nUTalkUin)
{
    ...
    std::pair<UINT, RMT_BUDDY_DATA*> data1 = std::make_pair(nUTalkUin, lpBuddyData);
   ...
   // RMT_BUDDY_DATA* lpBuddyData = NULL;
    ...
    if (NULL == lpBuddyData)
    {
        ...
        //std::pair<UINT, RMT_BUDDY_DATA*> data2 = std::make_pair(nUTalkUin1, lpBuddyData);
        pairInsert = m_mapBuddyData.emplace(std::make_pair(nUTalkUin1, lpBuddyData));
...
RMT_GROUP_DATA* CRecvMsgThread::GetGroupData(UINT nGroupCode)
{
   ...
   // RMT_GROUP_DATA* lpGroupData = NULL;
    ...
    if (NULL == lpGroupData)
    {
        ...
        pairInsert = m_mapGroupData.emplace(std::make_pair(nGroupCode, lpGroupData));
        if (!pairInsert.second)
        {
            delete lpGroupData;
            return NULL;
...
RMT_BUDDY_DATA* CRecvMsgThread::GetGMemberData(UINT nGroupCode, UINT nUTalkUin)
{
    ...
    RMT_BUDDY_DATA* data1 = NULL;
    CGMemberMapKey* data2 = NULL;
    ...
    if (NULL == lpGMemberData)
    {
        ...
        pairInsert = m_mapGMemberData.emplace(std::make_pair(key, lpGMemberData));
        if (!pairInsert.second)
        {
...
UINT CRecvMsgThread::GroupId2Code(UINT nGroupId)
{
    ...
    if (nGroupCode != 0)
    {
        pairInsert = m_mapGroupId2Code.emplace(std::make_pair(nGroupId, nGroupCode));
