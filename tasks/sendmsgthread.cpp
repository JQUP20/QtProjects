    ...
    while (!m_bStop)
    {
        CNetData* lpMsg;
        {
            ...
            while (m_listItems.empty())
            {
                ...
                std::cout << "start 2  ....  finish " << std::endl;
            }
...
{
//    CIUSocket *pIUSocket = new CIUSocket();

//    pIUSocket->m_Socket->write("AddItem");

    std::cout << "Ok, AddItem............" << std::endl;
    std::lock_guard<std::mutex> guard(m_mtItems);
    std::cout << "Ok, AddItem1............" << std::endl;
    m_listItems.push_back(pItem);
    std::cout << "Ok, AddItem2............" << std::endl;
    m_cvItems.notify_one();
    std::cout << "Ok, AddItem3............" << std::endl;
}

void CSendMsgThread::HandleItem(CNetData* pNetData)
{
    LOG_INFO("CSendMsgThread::HandleItem.....");
...
{
    LOG_INFO(" CSendMsgThread::HandleRegister.......");
    ...
    LOG_INFO("szRegisterInfo --> %s", szRegisterInfo);
    ...
    LOG_INFO("outbuf --> %s", outbuf.c_str());
...
{
    LOG_INFO(" CSendMsgThread::HandleLogon.......");
...
{
    LOG_INFO("CSendMsgThread::HandleUserBasicInfo(const CUserBasicInfoRequest* pUserBasicInfo)");
...
{
    LOG_INFO("CSendMsgThread::HandleFindFriendMessage ...");
...
void CSendMsgThread::HandleOperateFriendMessage(const COperateFriendRequest* pOperateFriendRequest)
{
    ...
    LOG_INFO("CSendMsgThread::HandleOperateFriendMessage ...");
    ...
    int len = data.length();
    int len1 = outbuf.length();
    LOG_INFO("data.length --> %d， data --> %s", len, data.c_str());
    LOG_INFO("outbuf.length --> %d, outbuf --> %s", len1, outbuf.c_str());
...
BOOL CSendMsgThread::AddBuddyMsg(UINT nFromUin, const tstring& strFromNickName, UINT nToUin, const tstring& strToNickName, time_t nTime, const tstring& strChatMsg, HWND hwndFrom /*= NULL*/)
{
    ...
    LOG_INFO("CSendMsgThread::AddBuddyMsg   ...... ");
    //TODO: 还是在主线程
