...
{
    LOG_INFO("CFile::Open(LPCTSTR lpszFileName, BOOL bCreateAlways/* = TRUE*/)");
    LOG_INFO("lpszFileName --> %s", lpszFileName);
    DWORD dwCreationFlag = (bCreateAlways ? CREATE_ALWAYS : OPEN_EXISTING);
    m_hFile = ::CreateFile(LPCTSTR(lpszFileName), GENERIC_READ|GENERIC_WRITE, FILE_SHARE_READ, NULL, dwCreationFlag, FILE_ATTRIBUTE_NORMAL, NULL);

    //m_hFile = ::CreateFile(_T("\\\\.\\test.txt"), GENERIC_READ|GENERIC_WRITE, FILE_SHARE_READ, NULL, dwCreationFlag, FILE_ATTRIBUTE_NORMAL, NULL);

    if(m_hFile == INVALID_HANDLE_VALUE)
    {
        LOG_INFO("m_hFile --> INVALID_HANDLE_VALUE");
    }
    else
    {
        LOG_INFO("m_hFile ！= INVALID_HANDLE_VALUE");
