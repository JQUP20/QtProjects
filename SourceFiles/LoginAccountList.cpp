...
{
	LOG_INFO("CLoginAccountList::LoadFile(LPCTSTR lpszFileName)");
...
{
	LOG_INFO("CLoginAccountList::SaveFile(LPCTSTR lpszFileName) ...");
	if (NULL == lpszFileName || NULL ==*lpszFileName)
		return FALSE;
...
{
	LOG_INFO(" CLoginAccountList::Add");
	if (NULL == lpszUser)
		return FALSE;
...
{
	LOG_INFO("CLoginAccountList::Del");
...
BOOL CLoginAccountList::Modify(int nIndex, LPCTSTR lpszUser, LPCTSTR lpszPwd, 
							   int nStatus, BOOL bRememberPwd, BOOL bAutoLogin)
{
	LOG_INFO("CLoginAccountList::Modify");
...
{
	LOG_INFO("CLoginAccountList::Clear()");
...
{
	LOG_INFO("CLoginAccountList::GetCount()");
...
{
	LOG_INFO(" CLoginAccountList::GetAccountInfo ");
	if (nIndex < 0 || nIndex >= (int)m_arrLoginAccount.size())
		return NULL;
...
{
	LOG_INFO("CLoginAccountList::Find(LPCTSTR lpszUser) ...");
	if (NULL == lpszUser)
		return NULL;
...
{
	LOG_INFO("CLoginAccountList::GetLastLoginUser() ...");
	return m_nLastLoginUser;
}

LOGIN_ACCOUNT_INFO* CLoginAccountList::GetLastLoginAccountInfo()
{
	LOG_INFO("CLoginAccountList:::GetLastLoginAccountInfo() ...");
...
{
	LOG_INFO("CLoginAccountList:::SetLastLoginUser ...");
	if (NULL == lpszUser)
		return;
...
{
	LOG_INFO("CLoginAccountList:::IsAutoLogin() ...");
...
{
	LOG_INFO("CLoginAccountList:::Encrypt() ...");
...
{
	LOG_INFO("CLoginAccountList:::Decrypt() ...");
	for (int i = 0; i < nLen; i++)
	{
		lpBuf[i] = lpBuf[i] ^ 0x88;
	}
...
{
	LOG_INFO("CLoginAccountList:::_fread ...");
...
{
	LOG_INFO("CLoginAccountList:::_fwrite ...");
