...
//    CFontInfo fontInfo = m_FontSelDlg.GetFontInfo();
    ...
    wsprintf(szFontInfo, lpFontFmt);
    strText += szFontInfo;

    m_LoginUserId = 1;
    m_strUserName = _T("Jack");

    m_UserId = 2;
    m_strBuddyName = _T("Mark");
...
void Chat::AddMsgToRecvEdit(time_t nTime, LPCTSTR lpText)
{
...
BOOL Chat::SendBuddyMsg(UINT nFromUin, const tstring &strFromNickName, UINT nToUin, const tstring &strToNickName, time_t nTime, const tstring &strChatMsg, HWND hwndFrom)
{
    return true;
}
