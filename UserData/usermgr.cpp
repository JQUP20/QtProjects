...
{
    LOG_INFO("CUserMgr::LoadRecentList()  ...... ");
...
{
    LOG_INFO("CUserMgr::LoadTeamInfo()  ...... ");
...
{
    LOG_INFO("CUserMgr::LoadBuddyInfo() ...... ");
    ...
    strBuddyInfoFilePath.Format(_T("%sUsers\\%s\\BuddyInfo.bi"), g_szHomePath, m_UserInfo.m_strAccount.c_str());
       ...
       // LOG_INFO("strBuddyInfoFilePath --> %s", strBuddyInfoFilePath);
        ...
        for(long i=0; i<nCount; ++i)
        {
            ...
            auto pr = std::make_pair(p->uAccountID, p);
            m_mapLocalBuddyInfo.emplace(pr);
            //m_mapLocalBuddyInfo.insert(std::make_pair(p->uAccountID, p));
        }
