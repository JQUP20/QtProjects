...
{
    LOG_INFO("CFlamingoClient::Uninit()");
...
{
    LOG_INFO("CFlamingoClient::Register(PCTSTR pszAccountName, PCTSTR pszNickName, PCTSTR pszPassword)");
    //TODO: 如果此时断网，则直接返回
...
{
    //TODO: 先判断一下是否离线
    LOG_INFO("CFlamingoClient::FindFriend(PCTSTR pszAccountName, long nType, HWND hReflectionWnd)");
...
{
    LOG_INFO("CFlamingoClient::AddFriend(UINT uAccountToAdd)");
    //TODO: 先判断是否离线
    if(uAccountToAdd == 0)
        return FALSE;
...
{
    LOG_INFO("CFlamingoClient::GetFriendList()");
...
// 发送好友消息
BOOL CFlamingoClient::SendBuddyMsg(UINT nFromUin, const tstring& strFromNickName, UINT nToUin,
                                   const tstring& strToNickName, time_t nTime, const tstring& strChatMsg,
                                   HWND hwndFrom/* = NULL*/)
{

    LOG_INFO("CFlamingoClient::SendBuddyMsg(UINT nFromUin, const tstring& strFromNickName, UINT nToUin   ...... ");
    return m_SendMsgThread->AddBuddyMsg(nFromUin, strFromNickName, nToUin, strToNickName,
                                        nTime, strChatMsg, hwndFrom);
}
...
void CFlamingoClient::OnLoginResult(UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    LOG_INFO("void CFlamingoClient::OnLoginResult(UINT message, WPARAM wParam, LPARAM lParam)");
...
{
    LOG_INFO("CFlamingoClient::OnUpdateUserBasicInfo(UINT message, WPARAM wParam, LPARAM lParam)   ...... ");
...
void CFlamingoClient::OnUpdateBuddyInfo(UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    //LOG_INFO("CFlamingoClient::OnUpdateBuddyInfo   lpBuddyInfo  -->  %s",  lpBuddyInfo->m_strAccount);
...
{
    LOG_INFO("void CFlamingoClient::LoadUserConfig()");
    ...
    for(size_t i=0; i<nTeamCount; ++i)
    {
        ...
        for(size_t j=0; j<nBuddyCount; ++j)
        {
            ...
            if(pBuddyInfo->m_nStatus != STATUS_OFFLINE)
                m_mapUserStatusCache.emplace(std::make_pair(pBuddyInfo->m_uUserID, pBuddyInfo->m_nStatus));
        }
    }
